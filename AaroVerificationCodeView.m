...
//#warning     此处要判断手机型号
    ...
    verificationCodeTextField.backgroundColor = AaroVerificationCodeTextFieldColor;
    verificationCodeTextField.frame = CGRectMake(0, 0, kCodeTextFieldWidth, kVerificationViewHeight);
    verificationCodeTextField.left = verticalLine.right + kCodeTextFieldLeftGap;
    verificationCodeTextField.placeholder = CodeTextFieldPlaceHolder;
    [leftView addSubview:verificationCodeTextField];
...
- (void)createCodeButton{
    ...
    codeButton.frame = CGRectMake(0, 0, kCodeButtonWidth, kVerificationViewHeight);
    codeButton.left = leftView.right + kCodeTextFieldRightWithButtonGap;
    [codeButton setBackgroundColor:[UIColor clearColor]];
    [codeButton.layer setBorderColor:AaroVerificationCodeButtonBorderColor];
    [codeButton.layer setBorderWidth:.8];
    [codeButton.layer setMasksToBounds:YES];
    [codeButton.layer setCornerRadius:AaroCornerRadius];
    
    [codeButton setTitle:@"验证码" forState:UIControlStateNormal];
    [codeButton setTitleColor:AaroVerificationCodeButtonColor forState:UIControlStateNormal];
//    #warning 屏幕适配
    if (kHeight <667) {
        codeButton.titleLabel.font = [UIFont boldSystemFontOfSize:12];//黑粗体
    }
    else{
        codeButton.titleLabel.font = [UIFont boldSystemFontOfSize:20];//黑粗体20
    }
