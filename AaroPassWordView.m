...
#define ChangeConfirmNewPlaceHolderString @"确认新密码(6-16位数字或字母)"
...
- (void)createPassWordTextField:(NSString *)placeHolder{
    ...
    passWordTextField.backgroundColor = AaroPassWordTextColor;
    passWordTextField.frame = CGRectMake(0, 0, kPassWordTextFieldWidth, kPassWordViewHeight);
    passWordTextField.left = verticalLine.right + kPassWordTextFieldLeftGap;
    [self addSubview:passWordTextField];
    passWordTextField.placeholder = placeHolder;
}
- (void)createOpenEyesIcon{
    openEyeButton = [[UIButton alloc] init];
    openEyeButton = [UIButton buttonWithType:UIButtonTypeCustom];
//#warning 判断iPhone大小
    if (kHeight <=480) {
        [openEyeButton setImage:[UIImage imageNamed:@"password_close"] forState:UIControlStateNormal];
    }
    else if (kHeight > 667) {
        [openEyeButton setImage:[UIImage imageNamed:@"password_close@3x"] forState:UIControlStateNormal];
    }else{
        [openEyeButton setImage:[UIImage imageNamed:@"password_close@2x"] forState:UIControlStateNormal];
    }
    
    
    openEyeButton.frame = CGRectMake(0, 0, kOpenEyeWidth, kOpenEyeHeight);
