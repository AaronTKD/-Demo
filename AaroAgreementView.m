...
- (void)createAgreementButton{
    ...
    agreementButton.frame = CGRectMake(kAgreementButtonLeftGap, informationLabel.bottom + kInformationLabelGapWithButton, kAgreementButtonWidth, kAgreementButtonHeight);

    
    [agreementButton setTitle:agreementButtonTitle forState:UIControlStateNormal];
    agreementButton.backgroundColor = [UIColor clearColor];
    
    
    [agreementButton setTitleColor:agreementButtonTitleColor forState:UIControlStateNormal];
//#warning 屏幕字体适配
    if (kHeight == 480) {
        agreementButton.titleLabel.font = [UIFont systemFontOfSize:6];
    }else if (kHeight == 568){
        agreementButton.titleLabel.font = [UIFont systemFontOfSize:9];
    }else{
